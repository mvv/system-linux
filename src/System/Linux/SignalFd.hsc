...
data Event = SigIllEvent { sigIllCode :: SigIllCode
             ...
             | UserEvent { userEventSignal :: Signal
                         , userEventUid :: UserID
                         , userEventPid :: ProcessID
                         }
             | KernelEvent { kernelEventSignal :: Signal }
             | QueuedEvent { queuedEventSignal :: Signal
                           , queuedEventUserData :: WordPtr
                           , queuedEventUid :: UserID
                           , queuedEventPid :: ProcessID
                           }
             | TimerEvent { timerEventSignal :: Signal
                          , timerEventTimer :: Timer
                          , timerEventOverrun :: CInt
                          , timerEventUserData :: WordPtr
                          }
             | MsgQueueEvent { msgQueueEventSignal :: Signal
                             , msgQueueUserData :: WordPtr
                             }
             | AsyncIOEvent { asyncIOEventSignal :: Signal }
             | IOEvent { ioEventSignal :: Signal }
             | ThreadEvent { threadEventSignal :: Signal }                          
               deriving (Eq, Show)
  ...
  poke p event = do
    ...
    case event of
      ...
      SigChldEvent code uid pid status utime stime -> do
        ...
        #{poke struct signalfd_siginfo, ssi_status} p status
...
readEvent fd = do
  let eventSize = sizeOf (undefined :: Event) in
    alloca $ \p -> do
...
foreign import ccall unsafe "signalfd"
  c_signalfd :: Fd -> Ptr () -> CreateFlags -> IO Fd
